...
def read_current_position(motor_id):
    ...
    response = send_command(command, motor_id)
    if response[0] == 0x3E and response[1] == motor_id:
        position = response[5] + (response[6] << 8) + (response[7] << 16) + (response[8] << 24)
...
def send_motor_command():
    ...
    # Leer y actualizar la posición actual
    current_position = read_current_position(motor_id)
    if current_position is not None:
        position_var.set(f"Posición Actual: {current_position:.2f}°")
...
ttk.Label(mainframe, text="ID del Motor:").grid(column=1, row=4, sticky=tk.EW, padx=10, pady=10)
ttk.Entry(mainframe, textvariable=motor_id_var).grid(column=2, row=4, sticky=(tk.EW), padx=10, pady=10)

ttk.Label(mainframe, text="Posición Objetivo (grados):").grid(column=1, row=5, sticky=tk.EW, padx=10, pady=10)
ttk.Entry(mainframe, textvariable=angle_var).grid(column=2, row=5, sticky=(tk.EW), padx=10, pady=10)

ttk.Label(mainframe, text="Velocidad Máxima (RPM):").grid(column=1, row=6, sticky=tk.EW, padx=10, pady=10)
ttk.Entry(mainframe, textvariable=speed_var).grid(column=2, row=6, sticky=(tk.EW), padx=10, pady=10)

ttk.Button(mainframe, text="Enviar Comando", command=send_motor_command, width=15).grid(column=3, row=6, sticky=tk.EW, padx=10, pady=10)

# Botones adicionales para control de posición predefinida
ttk.Button(mainframe, text="Posición Inicial", command=move_to_initial_position, width=15).grid(column=1, row=7, sticky=tk.EW, padx=10, pady=10)
ttk.Button(mainframe, text="Extensión", command=move_to_extension_position, width=15).grid(column=2, row=7, sticky=tk.EW, padx=10, pady=10)
ttk.Button(mainframe, text="Flexión", command=move_to_flexion_position, width=15).grid(column=3, row=7, sticky=tk.EW, padx=10, pady=10)

# Indicador de posición actual
ttk.Label(mainframe, textvariable=position_var).grid(column=1, row=8, columnspan=3, sticky=tk.EW, padx=10, pady=10)
